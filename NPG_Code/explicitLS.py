...
def run_lasso(m = m, n = n, seed = seed):
    ...
    def Run(algo, params = None, **kwargs):
        return algo(oracle_f, g, prox_g, x0, maxit = N, tol = tol, stop = "res", lns_init = True, verbose = False, 
                                track = ["res", "obj", "grad", "steps","time", "mse"], fixed_step = 0.001,tuning_params= params, xopt = xopt, **kwargs)
    ...
    if LOAD_RESULTS:
        ...
        for param in [[0.1, 5.7, 0.7, 0.69]]:
            x1, history1 = Run(NPG, param, ver=1)
            results["NPG1"] = history1

        for param in [[0.1, 5.7, 0.99, 0.98]]:
            x2, history2 = Run(NPG, param, ver=2)
            results["NPG2"] = history2

        for param in [[0.1, 5.7, 0.99, 0.98]]:
            x3, history3 = Run(NPG_quad, param)
        ...
        for param in [[1.1, 0.5], [1.2, 0.5]]:
            x6, history6 = Run(ProxGrad, param)
            results["PG-LS" + str(param)] = history6
        x7, history7 = Run(ProxGrad, params=[1.2, 0.6], LStype = 'explicit_LS1' )
        results["PG-eLS1" + str([1.2, 0.6])] = history7
        x8, history8 = Run(ProxGrad, params=[1.2, 0.6], LStype = 'explicit_LS2' )
        results["PG-eLS2" + str([1.2, 0.6])] = history8
